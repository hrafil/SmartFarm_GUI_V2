...
export default {
  ...
  putNewFarmObjectValue(farmObject) {
    if (DEBUG) {
      ...
      var conf_obj = FarmObjectsLocalStorage.configGetConfig();
      return axios.put(conf_obj.configuration.applicationGatewayUrl + "putFarmObjectValue", {
...
function arrangeDashboardExcludedIncluded(unregisteredObjects, listFarmObject) {
  ...
  for (var i = 0; i < filteredObjects.length; i++) {
    ...
    if (!isInIncluded) {
      ...
      includedObjects.push(filteredObjects[i])
    }
  }
...
function prepareData(farmObjectList, farmObjectValuesList) {
  for (var j = 0; j < farmObjectList.length; j++) {
    ...
    console.log(farmObjectList[j].name + "  " + value);
    if (value !== false) {

      if (curr_obj !== 0) {
        curr_obj.value = (Math.round(value) * 10) / 10;
      } else {
